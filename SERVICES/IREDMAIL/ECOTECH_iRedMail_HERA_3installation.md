...
# ATTENTION : le mot de passe exemple dans le fichier téléchargé peut varier.
# Toujours vérifier ce qui est réellement dans le fichier AVANT de faire le sed :
sudo grep "password" /etc/dovecot/dovecot.conf

# Remplacer le mot de passe exemple par le vrai
# Adapter MOT_DE_PASSE_EXEMPLE selon ce que grep a retourné ci-dessus
sudo sed -i 's/MOT_DE_PASSE_EXEMPLE/VOTRE_VRAI_MOT_DE_PASSE/' /etc/dovecot/dovecot.conf

# Vérifier que la connexion MariaDB fonctionne
sudo mysql -u vmailadmin -p'VOTRE_VRAI_MOT_DE_PASSE' vmail -e "SELECT 1;"
# Résultat attendu : | 1 |
...
> **Mot de passe exemple trouvé dans le fichier téléchargé :** `IdWPrT8a6Fwxc69zUCGB7TSryzbLRGAw`
>
> **Commande exacte appliquée sur EcoTech :**
> ```bash
> sudo sed -i 's/IdWPrT8a6Fwxc69zUCGB7TSryzbLRGAw/swRnasr2kqxRU1HRCt0GD0pIQn4aA3It/' /etc/dovecot/dovecot.conf
> ```
